...
class PublishedInSchema(BaseSchema):
    title = get_string_field(
        get_preflabel_lazy('title'), {'field_format': 'half', 'order': 1}
    )
    subtitle = get_string_field(
        get_preflabel_lazy('subtitle'), {'field_format': 'half', 'order': 2}
    )
...
class DocumentSchema(BaseSchema):
    ...
    isbn = get_string_field(
        get_preflabel_lazy('isbn'), {'field_format': 'half', 'order': 6}
    )
    doi = get_string_field(
        get_preflabel_lazy('doi'), {'field_format': 'half', 'order': 7}
    )
    ...
    volume = get_string_field(
        get_preflabel_lazy('volume_issue'), {'field_format': 'half', 'order': 10}
    )
    pages = get_string_field(
        get_preflabel_lazy('pages'), {'field_format': 'half', 'order': 11}
    )
    ...
    edition = get_string_field(
        get_preflabel_lazy('edition'), {'field_format': 'half', 'order': 16}
    )
