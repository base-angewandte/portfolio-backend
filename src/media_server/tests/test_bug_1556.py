...
During the asynchronous upload I changed the metadata, saved the
changes, but the button "View in Archive" did not change to the update
metadata state.
"""
...
class DeleteMediaJobs(APITestCase):
    def setUp(self) -> None:
        ...
        self.now += time_gone
        with freezegun.freeze_time(self.now):
            self.client.patch(path=f'/api/v1/entry/{self.entry.id}/', data={'title': 'New Title'})
